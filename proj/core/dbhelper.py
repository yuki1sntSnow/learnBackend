...
class DbHelper:
    ...
    async def select(self, kwargs: dict = None):
        ...
        a = await self.__filter(kwargs).first()
        return a
    ...
    async def update(self, filters: dict = None, updates: dict = None):
        ...
        a = await self.__filter(filters).update(**updates)
        return a
    ...
    async def delete(self, pk: int) -> int:
        ...
        a = await self.update(filters=filters, updates=updates)
        return a
    ...
    async def insert(self, data: dict):
        ...
        a = await self.model.create(**data)
        return a
    ...
    async def raw_sql(cls, sql: str, args: list = None):
        ...
        a = await db.execute_query_dict(sql, args)
        return a
...
async def has_roles(uid):
    ...
    a = await UserRoleDao.raw_sql(sql, [uid])
    return a
...
async def has_user(username):
    ...
    a = await UserDao.select({"username": username, "status__not": 9})
    return a
...
async def has_permissions(rid, is_menu=False):
    ...
    a = await RoleMenuDao.raw_sql(sql, [rid])
    return a