...
async def user_list(
    ...
    a = await UserService.get_items(offset, limit)
    return a
...
async def user_query(query: UserSchema.UserQuery):
    ...
    a = await UserService.query_items(query)
    return a
...
async def user_info(pk: int):
    ...
    a = await UserService.get_item(pk)
    return a
...
async def user_update(pk: int, data: UserSchema.UserPut):
    ...
    a = await UserService.update_item(pk, data)
    return a
...
async def user_change_role(
    ...
    a = await UserService.change_current_role(user.id, rid)
    return a 
